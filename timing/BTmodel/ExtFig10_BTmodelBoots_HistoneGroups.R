...
win_table_vcf = wintable_CN_plmstsSNV_X
mk_win_table <- function(win_table_vcf) {
  ...
  # actual make winning tables function:
...
VarCl4memb <- fread('/Users/fdubois/Dropbox/pHGG_data/data/20201217VarCorbyHistone/20201217metaDSig.txt')
table(VarCl4memb$Histone_group)
...
unique(wintable_CN_plmstsSNV$Sample)[!(unique(wintable_CN_plmstsSNV$Sample) %in% VarCl4memb$Tumor_Sample_Barcode)]
VarCl4memb$Tumor_Sample_Barcode <- gsub(pattern = '13630_S2', replacement = '13630_04A', x = VarCl4memb$Tumor_Sample_Barcode)
#VarCl4memb$Tumor_Sample_Barcode[!(VarCl4memb$Tumor_Sample_Barcode %in% unique(wintable_CN_plmstsSNV$Sample))]
...
colnames(VarCl4memb)
x = unique(VarCl4memb$Histone_group)[5]
lapply(unique(VarCl4memb$Histone_group), function(x){
  print(x)
  VarCl4membX <- VarCl4memb[Histone_group == x]
  ...
  write.table(bootBTm_gg_nNA_s, paste0('/Volumes/GoogleDrive/My Drive/FD_Beroukhim Lab/pHGG/data/20220316extDataFigs_sourceTbl/20220504extFig10/20220504extFig10_', x, '.txt'),quote=FALSE, sep='\t', row.names=FALSE, col.names = TRUE)
  
  pdf(file = paste0('/Volumes/GoogleDrive/My Drive/FD_Beroukhim Lab/pHGG/data/20220316extDataFigs_sourceTbl/20220504extFig10/20220504extFig10_', x, '_Boots100NFplotl2_t3_narms.pdf'), width = 16, height = 20)
